...
from offline_dataset_aug import OfflineNuScenesDatasetAugmented
...
def train_model():
    batch_size = 8  
    num_epochs = 10 # Augmented model takes much longer to memorize, but hits higher caps
    learning_rate = 1e-3
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n--- Heavy Augmented Deep Learning Engine Fired Up! Targeting Device: {device} ---")
    
    data_path = 'c:/drivableseg/exported_data_idd20k'
    print(f"Loading 640x360 Pre-Rendered Offline Dataset from: {data_path} with Aggressive Affine Augmentations!")
    
    try:
        full_dataset = OfflineNuScenesDatasetAugmented(data_dir=data_path, augment=True)
    ...
    print("Constructing EFFICIENTNET-B2 + DeepLabV3+ Heavy Architecture...")
    model = EfficientDeepLabV3Plus(num_classes=3).to(device)
    
    # -------------------------------------------------------------
    # Jumpstart Logic:
    # 1. Grab the latest NON-AUGMENTED weights as a starting block if we just began.
    # 2. But ALWAYS securely save into our newly isolated AUGMENTED .pth target.
    # -------------------------------------------------------------
    save_target_path = "c:/drivableseg/drivable_model_effnet_aug.pth"
    baseline_source_path = "c:/drivableseg/drivable_model_effnet_latest.pth"
    
    if os.path.exists(save_target_path):
        print(f"Resuming Augmented EfficientNet from existing targeted weights: {save_target_path}")
        model.load_state_dict(torch.load(save_target_path, map_location=device))
    elif os.path.exists(baseline_source_path):
        print(f"Jumpstarting Augmented training physically off the legacy baseline weights: {baseline_source_path}")
        model.load_state_dict(torch.load(baseline_source_path, map_location=device))
    ...
    print(f"\n[START] Commencing Aggressively Augmented EfficientNet Loop!")
    ...
    for epoch in range(num_epochs):
        ...
        # Save securely to our branched isolation line!
        torch.save(model.state_dict(), save_target_path)
        scheduler.step()

    print("\nTraining Phase Complete! Augmented Production Model weights securely stored to: ", save_target_path)
    
    print("Generating Augmented Matplotlib Tracking Graphs...")
    ...
    plt.title('Loss Trajectory over Epochs (Augmented Model)')
    plt.xlabel('Epochs')
    ...
    graph_path = os.path.join(graph_dir, "effnet_aug_training_metrics_graph.png")
